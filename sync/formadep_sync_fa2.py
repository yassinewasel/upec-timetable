...
"""
Synchronisation Formadep -> JSON pour l'emploi du temps FA2A / FA2C.

Utilisation locale à partir d'un HTML enregistré :
    python formadep_sync_fa2.py --html "formadep.html" --week 38 --out fa2_s38.json

Récupération directe depuis Formadep :
    python formadep_sync_fa2.py --week 38 --out fa2_s38.json

Plusieurs semaines :
    python formadep_sync_fa2.py --weeks 38 39 40 41 --out fa2.json
...
MAIN_URL = BASE_URL + "/multi/multi_edt?type=2&promo=73"
...
def _parse_first_line(first_line: str) -> tuple[str, str, str | None, str]:
    ...
    Compatible avec :
      ...
      FA2C-...
      Amphi-...
      blocs flash sans préfixe de groupe
    """
    ...
    m = re.fullmatch(r"FA2([ABC])", group_token)

    if not m:
        # Les blocs .flash de Formadep n'ont souvent pas FA2A/FA2B/FA2C
        # dans leur texte. Le groupe sera déterminé par leur position verticale.
        return "unknown", left_part, None, room
    ...
    if rest and re.fullmatch(rf"{re.escape(group_token)}[12]", rest[0]):
        subgroup = rest[0]
        rest = rest[1:]
...
def parse_formadep_html(html: str, week: int | None = None) -> dict[str, Any]:
    ...
    # Exemple : "Emploi du temps Semaine 38 - BUT Info ...".
    ...
    for div in soup.find_all("div"):
        ...
        # FA2 utilise à la fois des blocs "dd" et "flash".
        if "dd" not in classes and "flash" not in classes:
            continue
        ...
        # ------------------------------------------------------------
        # Cas particulier FA2 :
        # Les blocs "flash" n'indiquent pas toujours FA2A/B/C dans
        # leur texte. Leur ligne verticale indique le groupe :
        #
        #   première ligne  -> A
        #   deuxième ligne -> B
        #   troisième ligne-> C
        #
        # Un bloc d'environ 107 px de hauteur couvre les trois groupes.
        # ------------------------------------------------------------
        if group == "unknown" and "flash" in classes:
            if height >= 100:
                group = "shared"
            else:
                within_day = (top - Y_FIRST_DAY) - day_index * PX_PER_DAY
        ...
        # FA2 : uniquement A + C + cours communs.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Synchronisation EDT Formadep FA2A/FA2B/FA2C")
